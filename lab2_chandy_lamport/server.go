package chandy_lamport

import (
	"log"
	"sync"
)

type serverSnapshot struct {
	tokens    int
	messages  []*SnapshotMessage
	openLinks map[string]bool
	completed bool
}

// The main participant of the distributed snapshot protocol.
// Servers exchange token messages and marker messages among each other.
// Token messages represent the transfer of tokens from one server to another.
// Marker messages represent the progress of the snapshot process. The bulk of
// the distributed protocol is implemented in `HandlePacket` and `StartSnapshot`.
type Server struct {
	Id            string
	Tokens        int
	sim           *Simulator
	outboundLinks map[string]*Link // key = link.dest
	inboundLinks  map[string]*Link // key = link.src
	mu            sync.Mutex
	snapshots     map[int]*serverSnapshot
}

// A unidirectional communication channel between two servers
// Each link contains an event queue (as opposed to a packet queue)
type Link struct {
	src    string
	dest   string
	events *Queue
}

func NewServer(id string, tokens int, sim *Simulator) *Server {
	return &Server{
		id,
		tokens,
		sim,
		make(map[string]*Link),
		make(map[string]*Link),
		sync.Mutex{},
		make(map[int]*serverSnapshot),
	}
}

// Add a unidirectional link to the destination server
func (server *Server) AddOutboundLink(dest *Server) {
	if server == dest {
		return
	}
	l := Link{server.Id, dest.Id, NewQueue()}
	server.outboundLinks[dest.Id] = &l
	dest.inboundLinks[server.Id] = &l
}

// Send a message on all of the server's outbound links
func (server *Server) SendToNeighbors(message interface{}) {
	for _, serverId := range getSortedKeys(server.outboundLinks) {
		link := server.outboundLinks[serverId]
		server.sim.logger.RecordEvent(
			server,
			SentMessageEvent{server.Id, link.dest, message})
		link.events.Push(SendMessageEvent{
			server.Id,
			link.dest,
			message,
			server.sim.GetReceiveTime()})
	}
}

// Send a number of tokens to a neighbor attached to this server
func (server *Server) SendTokens(numTokens int, dest string) {
	if server.Tokens < numTokens {
		log.Fatalf("Server %v attempted to send %v tokens when it only has %v\n",
			server.Id, numTokens, server.Tokens)
	}
	message := TokenMessage{numTokens}
	server.sim.logger.RecordEvent(server, SentMessageEvent{server.Id, dest, message})
	// Update local state before sending the tokens
	server.Tokens -= numTokens
	link, ok := server.outboundLinks[dest]
	if !ok {
		log.Fatalf("Unknown dest ID %v from server %v\n", dest, server.Id)
	}
	link.events.Push(SendMessageEvent{
		server.Id,
		dest,
		message,
		server.sim.GetReceiveTime()})
}

// Callback for when a message is received on this server.
// When the snapshot algorithm completes on this server, this function
// should notify the simulator by calling `sim.NotifySnapshotComplete`.
func (server *Server) HandlePacket(src string, message interface{}) {
	switch msg := message.(type) {
	case TokenMessage:
		server.mu.Lock()
		for snapshotId, snapshot := range server.snapshots {
			if snapshot.openLinks[src] {
				snapshot.messages = append(snapshot.messages, &SnapshotMessage{
					src:     src,
					dest:    server.Id,
					message: msg,
				})
				server.snapshots[snapshotId] = snapshot
			}
		}
		server.Tokens += msg.numTokens
		server.mu.Unlock()
	case MarkerMessage:
		server.handleMarker(src, msg.snapshotId)
	default:
		log.Fatalf("Server %v received unknown message from %v: %v\n", server.Id, src, message)
	}
}

// Start the chandy-lamport snapshot algorithm on this server.
// This should be called only once per server.
func (server *Server) StartSnapshot(snapshotId int) {
	shouldSendMarkers, completed := server.startSnapshot(snapshotId)
	if shouldSendMarkers {
		server.SendToNeighbors(MarkerMessage{snapshotId})
	}
	if completed {
		server.sim.NotifySnapshotComplete(server.Id, snapshotId)
	}
}

func (server *Server) startSnapshot(snapshotId int) (bool, bool) {
	server.mu.Lock()
	defer server.mu.Unlock()

	if _, ok := server.snapshots[snapshotId]; ok {
		return false, false
	}

	openLinks := make(map[string]bool)
	for src := range server.inboundLinks {
		openLinks[src] = true
	}
	completed := len(openLinks) == 0
	server.snapshots[snapshotId] = &serverSnapshot{
		tokens:    server.Tokens,
		messages:  make([]*SnapshotMessage, 0),
		openLinks: openLinks,
		completed: completed,
	}
	return true, completed
}

func (server *Server) handleMarker(src string, snapshotId int) {
	shouldSendMarkers, completed := server.startSnapshot(snapshotId)
	if shouldSendMarkers {
		server.SendToNeighbors(MarkerMessage{snapshotId})
	}

	notify := false
	server.mu.Lock()
	snapshot := server.snapshots[snapshotId]
	delete(snapshot.openLinks, src)
	if len(snapshot.openLinks) == 0 && !snapshot.completed {
		snapshot.completed = true
		notify = true
	}
	server.mu.Unlock()

	if completed || notify {
		server.sim.NotifySnapshotComplete(server.Id, snapshotId)
	}
}

func (server *Server) getSnapshot(snapshotId int) *serverSnapshot {
	server.mu.Lock()
	defer server.mu.Unlock()

	snapshot := server.snapshots[snapshotId]
	copySnapshot := &serverSnapshot{
		tokens:    snapshot.tokens,
		messages:  make([]*SnapshotMessage, len(snapshot.messages)),
		openLinks: make(map[string]bool),
		completed: snapshot.completed,
	}
	copy(copySnapshot.messages, snapshot.messages)
	for src, open := range snapshot.openLinks {
		copySnapshot.openLinks[src] = open
	}
	return copySnapshot
}
